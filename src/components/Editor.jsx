  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      if (note.title !== draft.title || note.body !== draft.body || JSON.stringify(note.tags || []) !== JSON.stringify(draft.tags || [])) {
        try {
          setStatus('saving')
          await onChange(note.id, { title: draft.title, body: draft.body, tags: draft.tags })
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex gap-2 items-center">
            <div className="text-xs text-gray-500">
              ...
              {status === 'saved' && '저장됨'}
              {status === 'nosave' && '변경 없음'}
              {status === 'idle' && savedAt.current ? `마지막 저장: ${new Date(savedAt.current).toLocaleTimeString()}` : ''}
            </div>
          ...
          <div className="text-xs text-gray-400">{note.updatedAt ? `업데이트: ${new Date(note.updatedAt).toLocaleString()}` : ''}</div>
        </div>
      </div>
