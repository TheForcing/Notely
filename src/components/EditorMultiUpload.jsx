// src/components/EditorMultiUpload.jsx
import React, { useEffect, useState, useRef } from "react";
import { marked } from "marked";
import TagInput from "./TagInput";
import useAuth from "../hooks/useAuth";
import { startUploadUserFile } from "../firebase";

export default function Editor({
  note,
  onChange,
  onDelete,
  onTogglePin,
  addAttachmentMeta,
}) {
  const [draft, setDraft] = useState(
    note || { title: "", body: "", tags: [], attachments: [] }
  );
  const [status, setStatus] = useState("idle");
  const [uploads, setUploads] = useState([]); // {id, name, progress, task, status}
  const saveTimer = useRef(null);
  const dropRef = useRef(null);
  const { user } = useAuth();

  useEffect(
    () => setDraft(note || { title: "", body: "", tags: [], attachments: [] }),
    [note]
  );

  useEffect(() => {
    if (!note) return;
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(async () => {
      if (
        note.title !== draft.title ||
        note.body !== draft.body ||
        JSON.stringify(note.tags || []) !== JSON.stringify(draft.tags || [])
      ) {
        try {
          setStatus("saving");
          await onChange(note.id, {
            title: draft.title,
            body: draft.body,
            tags: draft.tags,
          });
          setStatus("saved");
          setTimeout(() => setStatus("idle"), 1000);
        } catch (e) {
          console.error(e);
          setStatus("nosave");
        }
      } else setStatus("idle");
    }, 700);
    return () => clearTimeout(saveTimer.current);
  }, [draft, note, onChange]);

  // start uploading multiple files
  const startFilesUpload = async (fileList) => {
    if (!note) return;
    if (!user) return alert("로그인이 필요합니다.");
    const arr = Array.from(fileList);
    for (const f of arr) {
      const uid = user.uid;
      const id =
        Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8);
      setUploads((u) => [
        { id, name: f.name, progress: 0, status: "uploading", task: null },
        ...u,
      ]);
      try {
        const { task, finished } = startUploadUserFile(uid, note.id, f, (p) => {
          setUploads((u) =>
            u.map((x) => (x.id === id ? { ...x, progress: p } : x))
          );
        });
        setUploads((u) => u.map((x) => (x.id === id ? { ...x, task } : x)));
        const meta = await finished;
        setDraft((s) => ({
          ...s,
          body: s.body + `\n![](${meta.url})\n`,
          attachments: [...(s.attachments || []), meta],
        }));
        try {
          await addAttachmentMeta(note.id, meta);
        } catch (e) {
          console.warn("addAttachmentMeta failed", e);
        }
        setUploads((u) =>
          u.map((x) =>
            x.id === id ? { ...x, progress: 100, status: "done" } : x
          )
        );
      } catch (e) {
        console.error("upload failed", e);
        setUploads((u) =>
          u.map((x) => (x.id === id ? { ...x, status: "error" } : x))
        );
      }
    }
  };

  // cancel upload
  const cancelUpload = (id) => {
    const item = uploads.find((u) => u.id === id);
    if (item && item.task && typeof item.task.cancel === "function") {
      try {
        item.task.cancel();
        setUploads((u) =>
          u.map((x) => (x.id === id ? { ...x, status: "cancelled" } : x))
        );
      } catch (e) {
        console.warn("cancel failed", e);
      }
    } else {
      setUploads((u) =>
        u.map((x) => (x.id === id ? { ...x, status: "cancelled" } : x))
      );
    }
  };

  // drag & drop handlers
  useEffect(() => {
    const el = dropRef.current;
    if (!el) return;
    const onDragOver = (e) => {
      e.preventDefault();
      el.classList.add("dropzone-active");
    };
    const onDragLeave = (e) => {
      e.preventDefault();
      el.classList.remove("dropzone-active");
    };
    const onDrop = (e) => {
      e.preventDefault();
      el.classList.remove("dropzone-active");
      const files = e.dataTransfer.files;
      if (files && files.length) startFilesUpload(files);
    };
    el.addEventListener("dragover", onDragOver);
    el.addEventListener("dragleave", onDragLeave);
    el.addEventListener("drop", onDrop);
    return () => {
      el.removeEventListener("dragover", onDragOver);
      el.removeEventListener("dragleave", onDragLeave);
      el.removeEventListener("drop", onDrop);
    };
  }, [dropRef.current, note, user, uploads]);

  // paste handler for images
  useEffect(() => {
    const onPaste = (e) => {
      if (!note) return;
      const items = e.clipboardData && e.clipboardData.items;
      if (!items) return;
      const files = [];
      for (const it of items) {
        if (it.kind === "file" && it.type.startsWith("image/")) {
          const f = it.getAsFile();
          if (f) files.push(f);
        }
      }
      if (files.length) startFilesUpload(files);
    };
    window.addEventListener("paste", onPaste);
    return () => window.removeEventListener("paste", onPaste);
  }, [note, user, uploads]);

  const onFileInput = (e) => {
    const files = e.target.files;
    if (files && files.length) startFilesUpload(files);
    e.target.value = "";
  };

  if (!note)
    return (
      <div className="editor">노트를 선택하거나 새 노트를 만들어주세요.</div>
    );

  return (
    <div className="editor">
      <div
        style={{
          display: "flex",
          justifyContent: "space-between",
          alignItems: "center",
          marginBottom: 12,
        }}
      >
        <input
          className="text-2xl font-semibold p-2 border rounded"
          value={draft.title}
          onChange={(e) => setDraft((s) => ({ ...s, title: e.target.value }))}
          placeholder="제목"
        />
        <div style={{ display: "flex", gap: 8 }}>
          <input type="file" multiple onChange={onFileInput} />
          <button onClick={() => onTogglePin(note.id)}>
            {note.pinned ? "Unpin" : "Pin"}
          </button>
          <button
            onClick={() => onDelete(note.id)}
            style={{ color: "#ef4444" }}
          >
            삭제
          </button>
        </div>
      </div>

      <div ref={dropRef} className={"textarea"}>
        <textarea
          className="textarea"
          value={draft.body}
          onChange={(e) => setDraft((s) => ({ ...s, body: e.target.value }))}
          placeholder="여기에 쓰거나 이미지를 붙여넣거나 드래그하세요"
        />
      </div>

      <div style={{ marginTop: 12 }}>
        <TagInput
          tags={draft.tags || []}
          onChange={(newTags) => setDraft((s) => ({ ...s, tags: newTags }))}
        />
      </div>

      <div className="upload-list">
        {uploads.map((u) => (
          <div key={u.id} className="upload-item">
            <div className="meta">
              <div style={{ fontSize: 13, fontWeight: 600 }}>{u.name}</div>
              <div className="progress" style={{ marginTop: 6 }}>
                <div style={{ width: u.progress + "%" }} />
              </div>
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>
                {u.status}
              </div>
            </div>
            {u.status === "uploading" && (
              <button onClick={() => cancelUpload(u.id)}>취소</button>
            )}
            {u.status === "error" && (
              <button
                onClick={() => {
                  /* retry: re-add file manually */
                }}
              >
                재시도
              </button>
            )}
          </div>
        ))}
      </div>
    </div>
  );
}
