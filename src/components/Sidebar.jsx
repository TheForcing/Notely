...
export default function Sidebar({ onCreate, search, setSearch, tagFilter, setTagFilter, exportNotes, importNotes }){
  ...
  return (
    <aside className="w-80 border-r bg-white p-4 flex flex-col gap-4">
      ...
      <input value={localSearch} onChange={e=>setLocalSearch(e.target.value)} placeholder="검색 (제목/본문/태그)" className="p-2 border rounded" />

      <div className="mt-2">
        <div className="text-xs text-gray-500 mb-1">태그 필터</div>
        <input value={tagFilter} onChange={e=>setTagFilter(e.target.value)} placeholder="태그로 필터(예: work)" className="p-2 border rounded w-full" />
      </div>
      ...
      <div>
        ...
        }} className="w-full py-2 border rounded mt-2">내보내기 (JSON)</button>
      ...
      <div className="text-xs text-gray-500 mt-2">Autosave • Markdown preview • Pins</div>
    </aside>
  )
}
