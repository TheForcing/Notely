...
export default function useNotes(){
  ...
  const createNote = useCallback(async () => {
    const newNote = { id: nanoid(), title: 'Untitled', body: '', createdAt: Date.now(), updatedAt: Date.now(), tags: [] };
  ...
  const updateNote = useCallback(async (id, patch) => {
    ...
    if(user){
      try {
        ...
        const toWrite = local ? { ...local, ...patch, updatedAt: Date.now() } : { id, ...patch, updatedAt: Date.now() };
  ...
  return { notes: filtered, rawNotes: notes, createNote, updateNote, deleteNote, activeNoteId, setActiveNoteId, search, setSearch };
}
